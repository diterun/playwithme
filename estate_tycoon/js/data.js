...
const GAME_DATA = {
  ...
  market: {
    ...
    // ★ propagate/sibling 은 "한 자원이 뿌리는 총합" 기준 — 엔진이 이웃 수로 나눠 적용한다(폭주 방지).
    dynamic: {
      enabled: true,
      stepSec: 600,           // 10분마다 가격 한 스텝 (그래프 histLen 10 → 최근 100분)
      ...
      propagate: 0.5,         // 원료 변화가 소비 제품으로 전파되는 총비율(이웃들이 나눠 가짐)
      sibling: 0.3,           // 같은 생산라인 형제 자원 동조 총비율(형제들이 나눠 가짐)
      chainDelaySteps: 2,     // 원료 변화가 제품에 전파되기까지 지연(스텝). 예: 나무↑ → 20분 뒤 판자↑
      siblingDelaySteps: 1,   // 형제 자원 동조 지연(스텝=10분)
      band: [0.5, 2.2],       // 기준가 대비 배수 하한·상한
      histLen: 10,            // 시세 그래프에 그릴 스텝 수(=최근 100분)
      catchUpMax: 720,        // 접속 시 오프라인 최대 진행 스텝(720×10분=5일). 넘으면 시각만 현재로 점프
    },
